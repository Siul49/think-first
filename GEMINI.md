...
- 시간대: Asia/Seoul
- 보고 형식: 브리핑 스타일 (배경→변경→영향→검증→차이→잔여). 상세: `.agent/skills/_shared/resources/report-template.md`
...
| 4 | **회고** | 작업 완료 후 사용자가 직접 회고 작성. `.agent/reflections/YYYY-MM-DD.md`에 기록 |
...
- 상세: `.agent/skills/_shared/resources/thinking-cycle.md`
...
커스텀 스킬은 `.agent/skills/`에 정의되어 있다. 맥락에 따라 자동 활성화된다.
...
`.agent/context/{task-id}/`에 문서 생성 (`task-id`: `{설명}-{YYYYMMDD}`):
...
`.agent/context/current-task.txt`에 task-id 기록. **사용자 승인 후 Phase 2 진입.**
...
백엔드 리더에게 보고하듯 구체적으로 브리핑한다. 상세: `.agent/skills/_shared/resources/report-template.md`
...
코드 스터디 후 사용자에게 회고 질문을 던진다. 사용자가 답하면 `.agent/reflections/YYYY-MM-DD.md`에 기록.
회고 없이 작업을 종료하지 않는다.
...
기존 독립 쉘 스크립트 스폰 방식은 사용하지 않으며, Antigravity **글로벌 워크플로우(`/workflows/`)** 또는 **MCP 서버 연계 플러그인** 형태로 다음 작업들을 위임한다:

| 상황 | 위임 대상 (예정) | 비고 |
|------|-------------|------|
| 코드 수정 완료 후 | `code-reviewer` | `/review` 워크플로우 실행 호출 |
| 복합 작업 감지 시 | `task-planner` | 기획서/체크리스트 생성 워크플로우 자동 반영 |
| 구현 완료 후 | `test-runner` | 백그라운드 테스트 워크플로우 |

### 워크플로우 전략

에이전트는 메인 트리에 직접 쓰기를 수행하는 `task-planner` 와, 일방향 피드백을 주는 `code-reviewer`를 기능 단위로 슬래시 커맨드(`/orchestrate`, `/review`)를 통해 호출하여 품질을 담당시켜야 한다.
...
- 상세 가이드: `.agent/korean-docs-style-guide.md`

## Git Hooks (이벤트 통제)

Antigravity 환경에서는 쉘 인터셉트 대신 `.github/hooks/pre-commit` 등 **네이티브 Git Hooks**를 통해 위험 명령 차단 및 자동 포매팅, 보안 스캔을 강제해야 한다. (에이전트 단의 `settings.json` 내 후킹 스크립트는 폐기됨)
