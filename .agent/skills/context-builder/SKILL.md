...
description: 프로젝트 컨텍스트 문서를 자동 생성하여 새 세션에서 AI가 빠르게 프로젝트를 이해하도록 돕습니다. "컨텍스트 정리", "프로젝트 요약", "온보딩 문서", "GEMINI.md 갱신" 요청 시 활성화됩니다.
---
...
| 문서 | 위치 | 용도 |
|------|------|------|
| GEMINI.md | 프로젝트 루트 | Antigravity 전역 설정 |
| 아키텍처 맵 | `.agent/context/architecture.md` | 디렉토리 구조, 레이어, 의존성 |
| 기술 스택 | `.agent/context/tech-stack.md` | 언어, 프레임워크, 주요 라이브러리 |
| 컨벤션 | `.agent/context/conventions.md` | 네이밍, 패턴, 코드 스타일 |
| 진행 현황 | `.agent/context/status.md` | 현재 진행 중인 작업, 알려진 이슈 |
...
- `.agent/context/` 디렉토리에 기존 문서가 있는지 확인
- 있으면 diff 기반 업데이트, 없으면 신규 생성
- GEMINI.md가 있으면 보존하고 보강 제안
...
2. **기존 문서 보존**: GEMINI.md 등 사용자가 작성한 내용은 절대 삭제하지 않음
...
| 파일 | 변경 내용 | 이유 |
|------|----------|------|
| `GEMINI.md` | 기술 스택 섹션 갱신 | React 18 → 19 업그레이드 반영 |
...
- GEMINI.md 템플릿: `antigravitymd-template.md`
