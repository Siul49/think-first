...
`.agent/skills/` 디렉토리를 일관되고 실행 가능한 상태로 유지한다.

1. 변경된 영역에 대한 검증 스킬 누락 탐지
2. 삭제/이동된 파일에 대한 잘못된 참조 탐지
3. 스킬 생성/업데이트/삭제 제안
4. `verify-implementation`, `GEMINI.md` 동기화
...
생성 후 **3곳 동기화**:
1. `.agent/skills/manage-skills/SKILL.md` — 등록 테이블
2. `.agent/skills/verify-implementation/SKILL.md` — 실행 목록
3. `GEMINI.md` — 스킬 테이블
