...
skill-pack의 `.agent/` 마이그레이션을 완성하고, 구조적 문제를 해결하여 설치 대상 프로젝트에서 모든 스킬이 정상 동작하도록 한다.

## 성공 기준

1. 모든 스킬의 `resources/` 참조가 실제 파일과 일치
2. context 경로가 `.agent/context/`로 통일
...
- resources/ 파일을 `.agent/` → `.agent/skills/*/resources/`로 마이그레이션
- antigravity.md, install.sh의 context 경로를 `.agent/context/`로 통일
- qa/review 스킬 역할 재정의
