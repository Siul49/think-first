...
skill-pack은 원래 Codex(`.agent/`) 기반으로 개발되었고, Antigravity(`.agent/`)로 마이그레이션 중이다. SKILL.md는 마이그레이션되었으나 resources/ 파일과 context 경로가 아직 레거시 상태.
...
- context 경로: `.agent/context/`로 통일 (`.agent/` 아래 일관성)
