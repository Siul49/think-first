...
1. `.agent/settings.json`에 Hooks 정의 → 설치 시 함께 복사됨
2. `.agent/agents/`에 Subagents 정의 → 스킬과 역할 분담 명확
...
`.agent/settings.json`에 hooks 섹션 추가.
프로젝트별로 포매터 명령을 커스터마이징할 수 있도록 주석 안내.

### 주의사항

- hooks는 `.agent/settings.json`(팀 공유)에 정의
- 개인용 override는 `.agent/settings.local.json`에서 가능
- hook 스크립트가 필요하면 `.agent/hooks/` 디렉토리에 배치
...
| **도구 제한** | 없음 (antigravity 전체 도구) | 명시적 화이트리스트 |
...
1. `.agent/hooks/` 디렉토리 생성
2. 위험 명령 차단 스크립트 작성
3. 체크리스트 리마인더 스크립트 작성

### Phase 2: Settings에 Hooks 등록
4. `.agent/settings.json` 생성 (hooks 섹션)
