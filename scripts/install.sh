...
#         bash scripts/install.sh /path/to/target-project --antigravity  [--with-config]
...
if [[ -z "$TARGET" || -z "$MODE" ]]; then
  ...
  echo "  bash scripts/install.sh <target-path> --antigravity  [--with-config]"
  echo "  bash scripts/install.sh <target-path> --plugin"
  echo ""
  echo "모드:"
  echo "  --claude        Claude Code용으로 설치 (.claude/skills/)"
  echo "  --antigravity   Antigravity/Gemini용으로 설치 (.agent/skills/)"
  ...
  echo "  bash scripts/install.sh ~/Dev/my-project --antigravity --with-config"
...
case "$MODE" in
  ...
  --antigravity)
    ...
    echo "[install] 모드: Antigravity/Gemini"
    ;;
  --plugin)
    BASE_DIR=".claude"
    SKILLS_DIR=".claude/skills"
    ...
    ;;
  *)
    echo "[error] 알 수 없는 모드: $MODE (--claude, --antigravity 또는 --plugin을 사용하세요)"
    exit 1
    ;;
esac
...
if [[ "$WITH_CONFIG" == "--with-config" ]]; then
  if [[ "$MODE" == "--claude" ]]; then
    ...
    # Antigravity: GEMINI.md 복사 (CLAUDE.md를 GEMINI.md로 변환)
    if [[ -f "$TARGET/GEMINI.md" ]]; then
      echo "[install] GEMINI.md가 이미 존재합니다. 덮어쓰지 않습니다."
      cp "$PACK_ROOT/CLAUDE.md" "$TARGET/GEMINI.md.think-first-template"
      echo "[install] 템플릿을 GEMINI.md.think-first-template로 저장합니다."
    else
      # CLAUDE.md 내용에서 경로만 치환하여 GEMINI.md로 저장
      sed 's|\.claude/skills/|.agent/skills/|g; s|\.claude/context/|.agent/context/|g' \
        "$PACK_ROOT/CLAUDE.md" > "$TARGET/GEMINI.md"
      echo "[install] GEMINI.md → 완료 (프로젝트에 맞게 수정하세요)"
    fi
  fi
fi
