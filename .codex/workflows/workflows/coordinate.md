...
- **Read the workflow-guide BEFORE starting.** Read `.codex/skills/workflow-guide/SKILL.md` and follow its Core Rules.
- **Follow the context-loading guide.** Read `.codex/skills/_shared/context-loading.md` and load only task-relevant resources.
...
3. Read `.codex/skills/_shared/memory-protocol.md` for memory protocol.
...
```powershell
powershell -ExecutionPolicy Bypass -File .codex/skills/_shared/ensure-big-task-docs.ps1 -Workspace . -Mode Init -TaskId <task-id>
```
...
4. Save plan to `.codex/plan.json`.
...
```powershell
powershell -ExecutionPolicy Bypass -File .codex/skills/_shared/ensure-big-task-docs.ps1 -Workspace . -Mode Review -TaskId <task-id> -SubtaskNote "<agent>/<task>"
```
