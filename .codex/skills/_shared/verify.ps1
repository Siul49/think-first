...
& (Join-Path $scriptDir "preflight.ps1") -Workspace $Workspace -AgentType $AgentType -SkillLockPath $SkillLockPath
if (-not $?) { exit 1 }
...
$skillLinkValidator = Join-Path $repoRoot ".codex/skills/manage-skills/scripts/validate-skill-links.ps1"
if (Test-Path $skillLinkValidator) {
    Invoke-Check "validate skill links" { & $skillLinkValidator | Out-Null }
}

$verifyRegistryValidator = Join-Path $repoRoot ".codex/skills/manage-skills/scripts/validate-verify-registry.ps1"
...
& (Join-Path $scriptDir "postflight.ps1") -Workspace $Workspace -AgentType $AgentType -SkillLockPath $SkillLockPath
if (-not $?) { exit 1 }
