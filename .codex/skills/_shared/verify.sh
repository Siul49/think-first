...
if command -v pwsh >/dev/null 2>&1; then
  if [[ -f "$repo_root/.codex/skills/manage-skills/scripts/validate-skill-links.ps1" ]]; then
    run_check "validate skill links" pwsh -NoProfile -ExecutionPolicy Bypass -File "$repo_root/.codex/skills/manage-skills/scripts/validate-skill-links.ps1"
  fi
  if [[ -f "$repo_root/.codex/skills/manage-skills/scripts/validate-verify-registry.ps1" ]]; then
    run_check "validate verify registry sync" pwsh -NoProfile -ExecutionPolicy Bypass -File "$repo_root/.codex/skills/manage-skills/scripts/validate-verify-registry.ps1" -RepoRoot "$repo_root"
  fi
fi
