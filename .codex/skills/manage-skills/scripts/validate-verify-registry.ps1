...
$requiredFiles = @($routingFile, $skillRegistryFile)
...
$registeredList = @(@($registry.skills) |
    Where-Object { $_.canonical_id -like "skill.verify.*" -and $_.canonical_id -ne "skill.verify.implementation" } |
    ForEach-Object {
        $_.canonical_id.ToString().Trim().ToLowerInvariant()
    } |
    Sort-Object -Unique)
...
if (-not (Compare-Sets -Label "catalog registry" -Actual $actualSkills -Declared $registeredList)) {
    $ok = $false
}
