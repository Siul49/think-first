...
백엔드 리더에게 보고하듯 구체적으로 브리핑한다. 상세: `.claude/skills/_shared/resources/report-template.md`
...
### 잔여 사항
- {남은 작업이나 경수님 확인 필요 사항} (없으면 생략)
```

#### Phase 5: 코드 스터디 (Thinking Cycle Phase 3)

보고 후, 변경된 코드를 체계적으로 학습한다. 전체 변경 스캔 → 체크리스트 → 항목별 What/Why/개념/연결/응용.
기반 개념까지 다루며, 이해 검증 루프로 설명 후 반드시 확인 질문. 코드 변경이 없는 작업은 생략.

#### Phase 6: 회고 (Thinking Cycle Phase 4)

코드 스터디 후 사용자에게 회고 질문을 던진다. 사용자가 답하면 `.claude/reflections/YYYY-MM-DD.md`에 기록.
회고 없이 작업을 종료하지 않는다.
...
10개 이벤트 Hook이 `settings.json`에 등록되어 시스템이 자동 실행한다. 상세는 README 참조.
